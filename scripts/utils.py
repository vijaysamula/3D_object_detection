...
def remove_low_score_nu(image_anno, thresh):
    ...
    pedestrain_indices =           get_annotations_indices(2, 0.1, label_preds_, scores_)
    cyclist_indices =         get_annotations_indices(3,0.1, label_preds_, scores_)
...
def get_xyz_points(cloud_array, translate =25,remove_nans=True, dtype=np.float):
    '''
    '''
    if remove_nans:
